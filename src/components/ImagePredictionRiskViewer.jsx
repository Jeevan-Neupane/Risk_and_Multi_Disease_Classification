...
function ImagePredictionRiskSwiper({ csvFilePath, imageDirPath }) {
  ...
  const isCorrect = (actual, predicted) => {
    return actual === predicted; // Compare actual vs predicted as direct values
  };

  return (
    <Container>
      <Title>Retinal Disease Risk Viewer</Title>
      <Swiper
        navigation={true}
        modules={[Navigation]}
        spaceBetween={30}
      >
        {records.map((row, index) => {
          const correct = isCorrect(row.actual_label, row.predicted_label);
          return (
            <SwiperSlide key={index}>
              <SlideContent>
                ...
                <Label className='actual'>
                  <span>Actual Label:</span> {row.actual_label || "—"}
                </Label>
                <Label className={correct ? "correct" : "incorrect"}>
                  <span>Predicted Label:</span> {row.predicted_label || "—"}
                </Label>
              </SlideContent>
            </SwiperSlide>
          );
...
export default ImagePredictionRiskSwiper;
